...
load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), "../MCP-Minecraft", ".env"))
...
if db_exists:
    ...
    PDF_1 = os.path.join(DATA_DIR, "안전보건교육_교재_한국에스웨이_10월_자료.pdf")
    PDF_2 = os.path.join(DATA_DIR, "자연재해대책법_시행규칙 (2).pdf")
...
# 외부에서 간단히 호출할 수 있는 함수
def get_answer(question: str) -> str:
    """RAG를 사용하여 질문에 대한 답변을 반환"""
    try:
        response = rag_chain.invoke(question)
        return response
...
if __name__ == "__main__":
    query_with_rag_invoke("한국에스웨이의 안전보건교육 교재에 따르면, 산업재해 예방을 위해 가장 중요한 요소는 무엇인가요?")
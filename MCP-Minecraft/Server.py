...
try:
    ...
    sys.stderr.write("[LLM] RAG 모듈 로드 성공\n")
except Exception as e:
    RAG_AVAILABLE = False
    RAG_IMPORT_ERROR = str(e)
    sys.stderr.write(f"[LLM] RAG 모듈 로드 실패: {e}\n")
...
# 컨테이너 이름 (기본값: gesture). 예전 코드와 호환되도록 선택 사항으로 둔다.
MOBIUS_CONTAINER = os.getenv("MOBIUS_CONTAINER", "gesture")
...
def show_stage_quiz_sync(stage_num: int) -> None:
    ...
    send_chat_message(
        mc, f"📍 STAGE {stage_num}: {stage['name']}", color="gold", bold=True
    )
    send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="gold")
    send_chat_message(mc, "")
...
KNOWN_GESTURES = set(GESTURE_TO_ANSWER.keys()) | {
    "Open_Palm",
    "Thumb_Down",
    "None",
    "Unknown",
...
def normalize_gesture(raw: str) -> str:
    """
    Mobius con 값이 'Closed_Fist,0.91' 처럼 confidence가 붙어 올 수 있으므로
    기본 제스처 라벨만 남기도록 정규화.
    """
...
def fetch_gesture_sync() -> str:
    """Mobius에서 제스처 가져오기 (RAW con 값 반환)"""
    try:
        url = (
            f"http://{MOBIUS_HOST}:{MOBIUS_PORT}/"
            f"{MOBIUS_CSE}/{MOBIUS_AE}/{MOBIUS_CONTAINER}/la"
        )
        headers = {
        ...
        if not cin:
            return "None"
        con_value = str(cin.get("con", "None"))
        return con_value
    except Exception as e:
...
def handle_llm_hint_request() -> str:
    ...
    try:
        ...
        stage_info = QUIZ_STAGES.get(current_quiz_stage)
        if not stage_info:
            return "퀴즈 진행 중이 아닙니다."
        ...
        if not RAG_AVAILABLE:
            ...
            sys.stderr.write(
                f"[LLM] RAG 모듈을 사용할 수 없습니다. 원인: {RAG_IMPORT_ERROR}\n"
            )
    ...
    try:
        ...
        if answer_num == correct_answer:
            ...
            if current_quiz_stage == 1:
                ...
                send_chat_message(
                    mc, "✓ [정답!] 잘하셨습니다!", color="green", bold=True
                )
                send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="green")
                send_chat_message(mc, "")
                send_chat_message(
                    mc, "📍 다음 장소로 이동하세요:", color="yellow", bold=True
                )
                send_chat_message(
                    mc,
                    f"   X={next_loc['x']}, Y={next_loc['y']}, Z={next_loc['z']}",
                    color="gold",
                )
                send_chat_message(mc, "")
                ...
                send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="gold")
                send_chat_message(
                    mc, "✓ [정답!] 완벽합니다!", color="green", bold=True
                )
                send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="gold")
                send_chat_message(mc, "")
                send_chat_message(
                    mc, "🎉 ★★★ CLEAR ★★★ 🎉", color="gold", bold=True
                )
                send_chat_message(
                    mc, "재난 안전 교육을 완료했습니다!", color="yellow"
                )
                send_chat_message(mc, "")
                send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="gold")
            ...
            send_chat_message(
                mc, "✗ [오답] 틀렸습니다!", color="red", bold=True
            )
            send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="red")
...
async def gesture_monitor():
    """제스처 자동 감지 및 퀴즈 처리"""
    last_raw = await asyncio.to_thread(fetch_gesture_sync)
    last_gesture = normalize_gesture(last_raw)
    sys.stderr.write(f"[Monitor] 초기 제스처 무시: {last_raw}\n")

    sys.stderr.write("[Monitor] 제스처 모니터링 시작...\n")

    while True:
        try:
            current_raw = await asyncio.to_thread(fetch_gesture_sync)
            current_gesture = normalize_gesture(current_raw)

            # 제스처 변경 감지 (정규화된 라벨 기준)
            if current_gesture != last_gesture and current_gesture not in [
                "None",
                "Unknown",
            ]:
                # 터미널에는 정규화된 제스처 이름만 출력
                sys.stderr.write(f"[Monitor] 감지: {current_gesture}\n")
                last_gesture = current_gesture  # 중복 방지
                ...
                # 퀴즈 진행 중일 때만 제스처 처리
                elif 0 < current_quiz_stage < 3:
                    result = await asyncio.to_thread(
                        process_quiz_gesture_sync, current_gesture
                    )
                    sys.stderr.write(f"[Monitor] 결과: {result}\n")

                    # Stage 1 정답 후 Stage 2 퀴즈 표시
...
async def start_quiz() -> str:
    ...
    try:
        ...
        send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="gold")
        send_chat_message(
            mc, "🚨  재난 안전 퀴즈 시작  🚨", color="yellow", bold=True
        )
        send_chat_message(mc, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", color="gold")
        send_chat_message(mc, "")
        send_chat_message(
            mc, "📌 제스처로 정답을 선택하세요:", color="green", bold=True
        )
        send_chat_message(mc, "")
        send_chat_message(
            mc, "   ✊ 주먹(Closed_Fist) = 0번", color="white"
        )
        send_chat_message(
            mc, "   ☝️ 검지(Pointing_Up) = 1번", color="white"
        )
        send_chat_message(
            mc, "   ✌️ 브이(Victory) = 2번", color="white"
        )
        send_chat_message(
            mc, "   🤟 아이러브유(ILoveYou) = 3번", color="white"
        )
        send_chat_message(
            mc, "   👍 엄지척(Thumb_Up) = 4번", color="white"
        )
        send_chat_message(mc, "")
        send_chat_message(
            mc, "💡 손바닥(Open_Palm) = AI 힌트 요청",
            color="light_purple",
            bold=True,
        )
        send_chat_message(mc, "")
        ...
        await asyncio.sleep(1.0)  # 마크 연결 대기
...
async def reset_quiz() -> str:
    ...
    try:
        ...
        if mc is not None:
            send_chat_message(mc, "")
            send_chat_message(
                mc, "🔄 [시스템] 퀴즈 초기화 완료", color="yellow", bold=True
            )
            send_chat_message(mc, "")
            return "퀴즈가 초기화되었습니다."
        else:
...
def quiz_workflow() -> str:
    ...
    - 검지(Pointing_Up) = 1번 
    - 브이(Victory) = 2번 
    - 아이러브유(ILoveYou) = 3번  ⭐ Stage 1 & 2 정답
    - 엄지척(Thumb_Up) = 4번
    - 손바닥(Open_Palm) = 도움 요청 (LLM)
    - 나머지(Thumb_Down 등) = 무시 또는 게임 시작 제어
    """
